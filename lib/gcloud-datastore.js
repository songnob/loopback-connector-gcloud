...
GcloudDataStoreConnector.prototype.find = function (model, filter, callback) {
   ...
   var filterClause = function (data, name) {
      ...
      if (idName == name) {
         ...
         if (data instanceof Object) {
            ...
            query     = query.filter(name, operator[key], data[key]);
         } else {
   ...
   // Show it
   debug(`find: execute query namespace: ${query.namespace}, kinds: ${JSON.stringify(query.kinds)},
    filters: ${JSON.stringify(query.filters)}, orders: ${JSON.stringify(query.orders)}`);
